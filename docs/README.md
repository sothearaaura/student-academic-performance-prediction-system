...
## 1.4. Security & deployment fixes (full-project audit)

- **`render.yaml`** no longer uses `preDeployCommand` — that's a paid-tier-only
  Render feature and was silently doing nothing on the Free plan anyway.
  `run.py` already seeds the database idempotently on every process boot,
  which is the correct mechanism for a host with an ephemeral filesystem.
- **`SECRET_KEY`** now fails loudly at startup if `DATABASE_URL` is set
  (i.e. a real deployment) but `SECRET_KEY` isn't — previously it silently
  fell back to a hardcoded key visible in this very source file, which
  would let anyone forge session cookies if accidentally deployed that way.
  Local SQLite development is unaffected; the fallback still works there.
- **CSRF protection** (Flask-WTF) is now enabled globally. All 20 POST forms
  across the app were updated with the required hidden token. Verified with
  a real request: a POST without a token is rejected (400); the actual
  rendered login form, with its real token, still works normally.
...
5-fold cross-validation (`sklearn.model_selection.cross_val_score`, run on
the exact deployed model + its exact feature/preprocessing pipeline) was
added specifically to confirm these numbers are robust across different
splits, not a single lucky train/test partition — see the `deployed_*_cv_*`
fields in `trained_models/metrics.json`.
...
- **Result** is now derived directly from **Predicted Grade** compared
  against the same Pass Threshold Level's tiers use — read live from
  `metrics.json` (the threshold the *currently deployed* classifier was
  actually trained with), never a separate hardcoded constant. This also
  means the admin Settings "Pass Threshold" value, the classifier's own
  training target, and the displayed Result can never drift out of sync.
...
- **Pass Probability / Confidence** remain the separately-trained
  classifier's own estimate — now clearly labeled "Classifier Est." with an
  explicit note that it's an independent signal, not what determines the
  Result shown. With the reconstructed dataset giving the classifier real
  signal to learn from, this estimate now naturally agrees with the
